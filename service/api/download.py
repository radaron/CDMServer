...
async def search_torrents(
    ...
    return JSONResponse(
        SearchResponse(
            data={
                "torrents": [
                    dump_torrent(torrent, availability) for torrent in result.torrents
                ]
            },
...
async def add_download(
    ...
    result = await session.execute(
        ...
        .where(
            user_device_association.c.user_id == user.id, Device.id == data.device_id
        )
        .with_for_update()
    )
    ...
    existing_files[data.torrent_id] = {
        "file_path": file_path,
