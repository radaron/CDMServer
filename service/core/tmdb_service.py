...
async def search_media(
    pattern: str, page: int = 1, language: str | None = None
) -> tuple[list[dict], int]:
...
async def popular_media(page: int = 1, language: str | None = None) -> dict:
    async with ClientSession(raise_for_status=True) as session:
        tmdb = get_tmdb(language, session=session)
        ...
        return {
            "movies": await asyncio.gather(
                *(fetch_with_limit(item, "movie") for item in movies.results or [])
            ),
            "tvs": await asyncio.gather(
                *(fetch_with_limit(item, "tv") for item in tvs.results or [])
            ),
        }


async def get_imdb_id(
    tmdb_id: int, media_type: str, language: str | None = None
) -> str | None:
    async with ClientSession(raise_for_status=True) as session:
        tmdb = get_tmdb(language, session=session)
